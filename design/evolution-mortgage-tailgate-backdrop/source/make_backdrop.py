...
from make_flag import (LOGO_D, LOGO_W, LOGO_H, COLOR_LOGO_URI, INT_SB, NAVY, GLOW, BLUE_DEEP, BLUE_CYAN, WHITE, html_wrap)
...
def emblem_markup(path, box, ink, dark):
    ...
    if path and os.path.exists(path):
        ...
        if ext == '.svg':
            body = open(path).read()
            return (f'<svg x="{-box/2}" y="{-box/2}" width="{box}" height="{box}" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">'
                    f'{body}</svg>')
...
def evo_markup(width, dark):
    s = width / LOGO_W; h = LOGO_H * s
    if dark:
        return f'<path d="{LOGO_D}" fill="{WHITE}" fill-rule="evenodd" transform="translate({-width/2:.4f},{-h/2:.4f}) scale({s:.6f})"/>'
...
def build(W=96.0, H=96.0, bleed=2.0, colorway='white', emblem=None, guides=False,
          evo_w=26.0, emblem_box=12.0, pitch_x=34.0, pitch_y=15.5):
    dark = colorway == 'navy'
    ...
    defs = [f'<g id="evo">{evo_markup(evo_w, dark)}</g>', f'<g id="emb">{emblem_markup(emblem, emblem_box, WHITE if dark else NAVY, dark)}</g>']
    ...
    d = (f'<defs><radialGradient id="glow" cx="0.5" cy="0.45" r="0.75"><stop offset="0" stop-color="{GLOW}"/><stop offset="1" stop-color="{NAVY}"/></radialGradient>'
